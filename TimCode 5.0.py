...
commands_dict = {
    ...
    'больще': 'Проверка "больше". Пример: если 10 больще 5. Аналог: >',
    ...
    'дробный_диалог', 'посткоманда',
            ...
            'module': {  
                'words': [
                    ...
                    'математика', 'корень', 'синус', 'косинус', 'тангенс', 'пи', 'случайность', 'комбинаторика', 'графический_интерфейс', 'ос', '',
...
#Кастомное меню ТимКода
    ...
    def _show_help_menu(self):
        ...
        self.edit_menu.add_command(label="Сайт TimCode", command=None)
    ...
    def _show_command_menu(self):
        ...
        self.edit_menu = tk.Menu(self, tearoff=0, bg='#2e2e2e', fg='white',
                                 activebackground='#4a4a4a', activeforeground='white')
        self.edit_menu.add_command(label="Список команд F2", command=self.show_completion)
...
# Функция для изменения цвета текста
    ...
    def translate_error(self, error_message: str) -> str:
        ...
        "NameError: name '.*' is not defined": "ОшибкаИмени: имя '\\g<1>' не определено",
        "UnboundLocalError: local variable '.*' referenced before assignment": "ОшибкаЛокальнойПеременной: локальная переменная '\\g<1>' используется до присваивания",
        "TypeError: unsupported operand type\\(s\\) for .*": "ОшибкаТипа: неподдерживаемые типы операндов для \\g<1>",
        "TypeError: '.*' object is not callable": "ОшибкаТипа: объект '\\g<1>' не является вызываемым",
        "TypeError: '.*' object is not iterable": "ОшибкаТипа: объект '\\g<1>' не является итерируемым",
        "ValueError: invalid literal for .*": "ОшибкаЗначения: неверное значение для \\g<1>",
        ...
        "KeyError: '.*'": "ОшибкаКлюча: ключ '\\g<1>' не найден",
        "AttributeError: '.*' object has no attribute '.*'": "ОшибкаАтрибута: объект '\\g<1>' не имеет атрибута '\\g<2>'",
        "FileNotFoundError: \\[Errno 2\\] No such file or directory: '.*'": "ФайлНеНайден: [Ошибка 2] Нет такого файла или каталога: '\\g<1>'",
        "PermissionError: \\[Errno 13\\] Permission denied: '.*'": "ОшибкаДоступа: [Ошибка 13] Доступ запрещен: '\\g<1>'",
        "IsADirectoryError: \\[Errno 21\\] Is a directory: '.*'": "ОшибкаКаталога: [Ошибка 21] Это каталог: '\\g<1>'",
        "FileExistsError: \\[Errno 17\\] File exists: '.*'": "ФайлУжеСуществует: [Ошибка 17] Файл уже существует: '\\g<1>'",
        "ImportError: No module named '.*'": "ОшибкаИмпорта: нет модуля с именем '\\g<1>'",
        "ModuleNotFoundError: No module named '.*'": "МодульНеНайден: нет модуля с именем '\\g<1>'",
        ...
        "IOError: \\[Errno .*\\] .*": "ОшибкаВводаВывода: [Ошибка \\g<1>] \\g<2>",
        "RuntimeError: .*": "ОшибкаВыполнения: \\g<1>",
        "NotImplementedError": "МетодНеРеализован",
        "StopIteration": "ИтерацияОстановлена",
        "Warning: .*": "Предупреждение: \\g<1>",
        "ResourceWarning: unclosed file <_io.TextIOWrapper name='.*' mode='.*' encoding='.*'>": "ПредупреждениеОРесурсе: файл '\\g<1>' не был закрыт",
        }
